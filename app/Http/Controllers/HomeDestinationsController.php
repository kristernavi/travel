...
class HomeDestinationsController extends Controller
{
    public function index()
    {
        ...
        if (strlen($keyword) > 1) {
            ...
            $details = PackageDetails::select(DB::raw('id,package_id,destination_id,user_id,SUM(price) as total_price'))->whereIn('destination_id', $destinations)->uniquePackage()->get();
        } else {
            $details = PackageDetails::select(DB::raw('id,package_id,destination_id,user_id,SUM(price) as total_price'))->inRandomOrder()->uniquePackage()->take(10)->get();
        }
